...
public sealed class ModeloController : Controller
{
    ...
    public async Task<IActionResult> GetAll(long id)
    {
        ...
        if (modelo == null)
            return NotFound(new NotFoundException($"Modelo não foi encontrado pelo id: {id}"));
    ...
    public async Task<IActionResult> Post([FromBody] ModeloDTO value)
    {
        ...
        if (exists != null)
            return Conflict(new ConflictException("Nome do modelo já existe, escolha outro."));

        var collection = await UseCase.Create(value);
        if (collection == null)
            return BadRequest(
                new BadRequestException(
                    "Algo de errado aconteceu durante a requisição da criação de modelo."
                )
            );
    ...
    public async Task<IActionResult> Put(long id, [FromBody] ModeloParams @params)
    {
        ...
        if (modelo == null)
            return NotFound(
                new NotFoundException($"Modelo não foi encontrada pelo Identificador {id}.")
            );
    ...
    public async Task<IActionResult> Put(long id, [FromBody] Status status)
    {
        try
        {
            ...
            if (modelo.Status.ToLower() == status.Value.ToLower())
                return Conflict(new ConflictException($"Coleção já está {modelo.Status}"));
    ...
    public async Task<IActionResult> Delete(long id)
    {
        try
        {
            ...
            if (modelo == null)
                return NotFound(
                    new NotFoundException($"Modeol não foi encontrado pelo Identificador {id}.")
                );
            if (modelo.Status.ToLower() == "ativo")
                return BadRequest(
                    new NotFoundException(
                        $"Modelo de Id {id} está ativo. Só é possível deletar modelos inativos."
                    )
                );
