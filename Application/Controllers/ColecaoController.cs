...
public sealed class ColecaoController : Controller
{
    ...
    public async Task<IActionResult> GetAll(long id)
    {
        ...
        if (colecao == null)
            return NotFound(new NotFoundException($"Coleção não foi encontrada pelo id: {id}"));
    ...
    public async Task<IActionResult> Post([FromBody] ColecaoDTO value)
    {
        ...
        if (exists != null)
            return Conflict(new ConflictException("Nome da coleção já existe, escola outro."));

        var collection = await UseCase.Create(value);
        if (collection == null)
            return BadRequest(
                new BadRequestException(
                    "Algo de errado aconteceu durante a requisição da criação de coleções."
                )
            );
    ...
    public async Task<IActionResult> Put(long id, [FromBody] ColecaoParams @params)
    {
        ...
        if (colecao == null)
            return NotFound(
                new NotFoundException("Colecao não foi encontrada pelo Identificador.")
            );
    ...
    public async Task<IActionResult> Put(long id, [FromBody] Status status)
    {
        try
        {
            ...
            if (colecao == null)
                return NotFound(
                    new NotFoundException("Coleção não foi encontrado pelo Identificador.")
                );
            if (colecao.Status.ToLower() == status.Value.ToLower())
                return Conflict(new ConflictException($"Coleção já está {colecao.Status}"));
    ...
    public async Task<IActionResult> Delete(long id)
    {
        try
        {
            ...
            if (colecao == null)
                return NotFound(
                    new NotFoundException($"Coleção não foi encontrado pelo Identificador {id}.")
                );
            if (colecao.Status.ToLower() == "ativo")
                return BadRequest(
                    new NotFoundException(
                        $"Coleção de Id {id} está ativa. Só é possível deletar coleções inativas."
                    )
                );
