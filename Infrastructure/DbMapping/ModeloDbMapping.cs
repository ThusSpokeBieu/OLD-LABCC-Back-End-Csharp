...
public class ModeloDbMapping
{
    public void Build(ModelBuilder builder)
    {
        ...
        builder.Entity<Modelo>().HasIndex(m => m.NomeDoModelo).IsUnique();

        builder.Entity<Modelo>().HasOne(m => m.Colecao).WithMany().HasForeignKey(m => m.ColecaoId);

        builder.Entity<Modelo>().HasOne(m => m.Layout).WithMany().HasForeignKey(m => m.LayoutId);
