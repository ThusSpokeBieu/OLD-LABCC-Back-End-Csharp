...
public sealed class UsuarioRepository : IUsuarioRepository
{
    ...
    public IQueryable<Usuario> Select() =>
        _db.Usuarios.Include(u => u.Genero).Include(u => u.Status).Include(u => u.TipoDeUsuario);

    public async Task<Usuario?> Select(string id) => await _db.Usuarios.FindAsync(id);
    ...
    public IQueryable<Usuario> Select(UsuarioParams @params)
    {
        ...
        if (@params.Id.IsNullOrEmpty())
            query = query.Where(u => u.Id.Equals(@params.Id));

        if (!@params.Nome.IsNullOrEmpty())
            query = query.Where(u => u.Nome.ToLower().Equals(@params.Nome!.ToLower()));

        if (!@params.Email.IsNullOrEmpty())
            query = query.Where(u => u.UserName.ToLower().Equals(@params.Email!.ToLower()));

        if (!@params.CpfOuCnpj.IsNullOrEmpty())
            query = query.Where(
                u => u.CpfOuCnpj.Equals(NotNumeric.Replace(@params.CpfOuCnpj!, ""))
            );

        if (!@params.Telefone.IsNullOrEmpty())
            query = query.Where(u => u.Telefone.Equals(NotNumeric.Replace(@params.Telefone!, "")));

        if (!@params.Genero.IsNullOrEmpty())
            query = query.Where(u => u.Genero!.Value.ToLower().Equals(@params.Genero!.ToLower()));

        if (!@params.Status.IsNullOrEmpty())
            query = query.Where(u => u.Status!.Value.ToLower().Equals(@params.Status!.ToLower()));

        if (!@params.TipoDeUsuario.IsNullOrEmpty())
            query = query.Where(
                u => u.TipoDeUsuario!.Value.ToLower().Equals(@params.TipoDeUsuario!.ToLower())
            );

        if (@params.TipoDeUsuarioId.HasValue)
            query = query.Where(u => u.TipoDeUsuarioId == @params.TipoDeUsuarioId);

        if (@params.StatusId.HasValue)
            query = query.Where(u => u.StatusId == @params.StatusId);

        if (@params.GeneroId.HasValue)
            query = query.Where(u => u.GeneroId == @params.GeneroId);
    ...
    public async Task Update(string id, UsuarioParams @params)
    {
        Usuario? user =
        ...
        if (!@params.Genero.IsNullOrEmpty())
            user.GeneroId = (byte)
                (GeneroEnum)StringEnumConverter.StringToEnum(@params.Genero!, typeof(GeneroEnum));
    ...
    {
        Usuario? user =
            await _db.Usuarios.FirstOrDefaultAsync(user => user.Id == id)
    ...
    public async Task Activate(string id)
    {
        ...
        if (user.StatusId == 1)
            throw new ArgumentException(
                $"The user: ${user.Nome}, id: ${user.Id} is already active."
            );
