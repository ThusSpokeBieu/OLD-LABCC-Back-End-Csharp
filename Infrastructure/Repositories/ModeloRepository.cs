...
public sealed class ModeloRepository : IModeloRepository
{
    ...
    public IQueryable<Modelo> Select() =>
        _db.Modelos
    ...
    public async Task<Modelo?> Select(long id) =>
    ...
    public IQueryable<Modelo> Select(ModeloParams? @params)
    {
        ...
        if (!@params.NomeDoModelo.IsNullOrEmpty())
            query = query.Where(
                m => m.NomeDoModelo!.ToLower().Equals(@params.NomeDoModelo!.ToLower())
            );

        if (@params.ColecaoId.HasValue)
            query = query.Where(m => m.ColecaoId == @params.ColecaoId);

        if (!@params.Colecao.IsNullOrEmpty())
            query = query.Where(
                m => m.Colecao!.NomeDaColecao!.ToLower().Equals(@params.Colecao!.ToLower())
            );

        if (!@params.Status.IsNullOrEmpty())
            query = query.Where(m => m.Status!.Value.ToLower().Equals(@params.Status!.ToLower()));

        if (@params.StatusId.HasValue)
            query = query.Where(m => m.StatusId == @params.StatusId);

        if (!@params.TipoDeModelo.IsNullOrEmpty())
            query = query.Where(
                m => m.TipoDeModelo!.Value.ToLower().Equals(@params.TipoDeModelo!.ToLower())
            );

        if (@params.TipoDeModeloId.HasValue)
            query = query.Where(m => m.TipoDeModeloId == @params.TipoDeModeloId);

        if (!@params.Layout.IsNullOrEmpty())
            query = query.Where(m => m.Layout!.Value.ToLower().Equals(@params.Layout!.ToLower()));

        if (@params.LayoutId.HasValue)
            query = query.Where(m => m.LayoutId == @params.LayoutId);
    ...
    public async Task Insert(Modelo obj)
    {
        try
        {
    ...
    public async Task Update(long id, ModeloParams @params)
    {
        Modelo? modelo =
            await Select(id)
            ?? throw new ArgumentException($"Modelo não foi encontrada pelo ID {id}");
        ...
        if (!@params.Layout.IsNullOrEmpty())
            modelo.LayoutId = (byte)
                (ModeloLayoutEnum)
                    StringEnumConverter.StringToEnum(@params.Layout!, typeof(ModeloLayoutEnum));
