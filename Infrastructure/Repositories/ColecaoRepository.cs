...
public sealed class ColecaoRepository : IColecaoRepository
{
  ...
  public IQueryable<Colecao> Select() => _db.Colecoes
      .Include(c => c.Estacao)
  ...
  public async Task<Colecao?> Select(long id) => await Select()
  ...
  public IQueryable<Colecao> Select(ColecaoParams? @params)
  {
    ...
    if (!@params.NomeDaColecao.IsNullOrEmpty()) query = query
        .Where(c => c.NomeDaColecao.ToLower().Equals(@params.NomeDaColecao!.ToLower()));

    if (!@params.Marca.IsNullOrEmpty()) query = query
        .Where(c => c.Marca.ToLower().Equals(@params.Marca!.ToLower()));

    if (@params.ResponsavelId.HasValue) query = query
        .Where(c => c.ResponsavelId == @params.ResponsavelId);

    if (!@params.Status.IsNullOrEmpty()) query = query
        .Where(c => c.Status.Value.ToLower().Equals(@params.Status!.ToLower()));

    if (@params.StatusId.HasValue) query = query
        .Where(c => c.StatusId == @params.StatusId);

    if (!@params.Estacao.IsNullOrEmpty()) query = query
        .Where(c => c.Estacao.Value
            .ToLower().Equals(@params.Estacao!.ToLower()));

    if (@params.AnoDeLancamento.HasValue) query = query
        .Where(c => c.AnoDeLancamento == @params.AnoDeLancamento);
  ...
  async public Task Insert(Colecao obj)
  {
    try
    {
  ...
  async public Task Update(long id, ColecaoParams @params)
  {
    Colecao? colecao = await Select(id) ?? throw new ArgumentException("Coleção não foi encontrada pelo ID");
    ...
    if (!@params.Status.IsNullOrEmpty())
      colecao.StatusId = (byte)(StatusEnum) StringEnumConverter
        .StringToEnum(@params.Status!, typeof(StatusEnum));

    if (!@params.Estacao.IsNullOrEmpty())
      colecao.EstacaoId = (byte)(EstacoesEnum) StringEnumConverter
        .StringToEnum(@params.Estacao!, typeof(EstacoesEnum));
